...
class PremiseFormElements {
		...
		switch ( $this->field['type'] ) {
			case 'checkbox':
				...
				<div class="'.$this->field['type'].'">
					<input type="'.$this->field['type'].'" '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$this->field['id'].'" value="'.$this->field['value_att'].'" '.$active.'>
				...
				<div class="'.$this->field['type'].'">';
					if( !empty( $this->field['options'] ) && is_array( $this->field['options'] ) ) {
						foreach ($this->field['options'] as $radio) {
							...
							echo '<input type="'.$this->field['type'].'" '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$radio['id'].'" value="'.$radio['value_att'].'" '.$active.'>';
							echo '<label for="'.$radio['id'].'">'.$radio['label'].'</label>';
						}
					}
					else {
						$active = checked( $this->field['value'], $this->field['value_att'], false );
						echo '<input type="'.$this->field['type'].'" '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$this->field['id'].'" value="'.$this->field['value_att'].'" '.$active.'>';
			...
			case 'file':
				echo '
				<div class="'.$this->field['type'].'">
					<input type="text" '.$placeholder.' '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$this->field['id'].'" value="'.$this->field['value'].'" class="premise-file-url">
			...
			case 'color':
				echo '
				<div class="'.$this->field['type'].'">
					<input type="'.$this->field['type'].'" '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$this->field['id'].'" value="'.$this->field['value'].'"'.$placeholder.'>
				</div>';
				break;
	...
	public function select_field() {
		...
		if( $this->field['type'] == 'wp_dropdown_pages' ) {
			...
			echo '<select '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$this->field['id'].'">';
			echo $this->field['placeholder'] ? '<option>'.$this->field['placeholder'].'</option>' : '';
			$this->options();
			echo '</select>';
		}
		echo '</div>';
	...
	public function textarea() {
		...
		echo '
		<div class="'.$this->field['type'].'">
			<textarea '.$this->field['attribute'].' name="'.$this->field['name'].'" id="'.$this->field['id'].'"'.$placeholder.'>'.$this->field['value'].'</textarea>
		</div>';
	}
	...
	public function fa_icons() {
		...
		foreach ($fa_icons as $icon) 
			echo '<li class="inline-block float-left"><a href="javascript:;" class="this-icon" '.$this->field['attribute'].' data-icon="'.$icon.'"><i class="fa fa-fw '.$icon.'"></i></a></li>';
		echo '</ul></div>';
	}
	...
	public function begin_field() {
		...
		if( $this->field['container'] && !empty( $this->field['container_title'] ) )
			echo '<h3>'.$this->field['container_title'].'</h3>';

		if( $this->field['container'] && !empty( $this->field['container_desc'] ) )
			echo '<p>'.$this->field['container_desc'].'</p>';
	}
